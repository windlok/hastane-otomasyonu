...
<body>
    ...
    <div class="page-container">
        <div class="page-header">
            <h2>🔧 Admin Paneli</h2>
            <p>Sistemi yönetin, kullanıcıları ve randevuları kontrol edin.</p>
        </div>
        ...
        <div class="admin-tabs">
            <a href="?tab=dashboard" class="admin-tab <?php echo $aktif_sekme === 'dashboard' ? 'active' : ''; ?>">📊 Genel Bakış</a>
            <a href="?tab=kullanicilar" class="admin-tab <?php echo $aktif_sekme === 'kullanicilar' ? 'active' : ''; ?>">👥 Kullanıcılar</a>
            <a href="?tab=randevular" class="admin-tab <?php echo $aktif_sekme === 'randevular' ? 'active' : ''; ?>">📅 Randevular</a>
            <a href="?tab=doktor-ekle" class="admin-tab <?php echo $aktif_sekme === 'doktor-ekle' ? 'active' : ''; ?>">👨‍⚕️ Doktor Ekle</a>
        </div>

        <?php if ($aktif_sekme === 'dashboard'): ?>
        <!-- Dashboard -->
        <div class="stat-row" style="margin-bottom: 25px;">
            <div class="stat-card"><div class="stat-icon">👤</div><div class="stat-value"><?php echo $toplam_kullanici; ?></div><div class="stat-label">Hasta</div></div>
            <div class="stat-card"><div class="stat-icon">👨‍⚕️</div><div class="stat-value"><?php echo $toplam_doktor; ?></div><div class="stat-label">Doktor</div></div>
            <div class="stat-card"><div class="stat-icon">📅</div><div class="stat-value"><?php echo $toplam_randevu; ?></div><div class="stat-label">Toplam Randevu</div></div>
            <div class="stat-card"><div class="stat-icon">✅</div><div class="stat-value"><?php echo $aktif_randevu; ?></div><div class="stat-label">Aktif Randevu</div></div>
        </div>

        <div class="card">
            <h3 class="card-title"><span class="card-icon">📋</span> Son 20 Randevu</h3>
            <div class="table-container">
                <table>
                    <thead><tr><th>Hasta</th><th>Doktor</th><th>Klinik</th><th>Tarih</th><th>Saat</th><th>Durum</th></tr></thead>
                    <tbody>
                        <?php if (empty($son_randevular)): ?>
                        <tr><td colspan="6" class="empty-state">Henüz randevu yok.</td></tr>
                        <?php else: ?>
                        <?php foreach ($son_randevular as $r): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($r['kullanici_adsoyad'] ?? 'Silinmiş'); ?></td>
                            ...
                            <td><span style="color: <?php echo $r['durum'] === 'aktif' ? 'var(--success)' : '#e74c3c'; ?>; font-weight:600;"><?php echo ucfirst($r['durum'] ?? 'aktif'); ?></span></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
        ...
        <!-- Kullanıcı Yönetimi -->
        <div class="card">
            <h3 class="card-title"><span class="card-icon">👥</span> Tüm Kullanıcılar</h3>
            <div class="table-container">
                <table>
                    <thead><tr><th>Ad Soyad</th><th>TC</th><th>E-posta</th><th>Telefon</th><th>Rol</th><th>Durum</th><th>İşlem</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span style="color: <?php echo $k['aktif'] ? 'var(--success)' : '#e74c3c'; ?>; font-weight:600;"><?php echo $k['aktif'] ? 'Aktif' : 'Pasif'; ?></span></td>
                            <td>
                                <?php if ($k['rol'] !== 'admin'): ?>
                                <form action="islem.php" method="post" style="display:inline;">
                                    ...
                                    <button type="submit" name="admin_kullanici_toggle" class="btn-sm" style="background:<?php echo $k['aktif'] ? '#fee' : '#e8f5e9'; ?>; color:<?php echo $k['aktif'] ? '#c62828' : '#2e7d32'; ?>;">
                                        <?php echo $k['aktif'] ? 'Pasif Yap' : 'Aktif Yap'; ?>
                                    </button>
                                </form>
                                <?php else: ?>
        ...
        <!-- Tüm Randevular -->
        <div class="card">
            <h3 class="card-title"><span class="card-icon">📅</span> Tüm Randevular</h3>
            <div class="table-container">
                <table>
                    <thead><tr><th>Hasta</th><th>Doktor</th><th>Klinik</th><th>Tarih</th><th>Saat</th><th>Durum</th></tr></thead>
                    <tbody>
                        ...
                        try {
                            ...
                            echo "<tr><td colspan='6' class='empty-state'>Veri yüklenemedi.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
        ...
        <div class="dashboard-grid">
            <div class="card">
                <h3 class="card-title"><span class="card-icon">👨‍⚕️</span> Yeni Doktor Ekle</h3>
                <form action="islem.php" method="post">
                    ...
                    <button type="submit" class="btn btn-primary" name="admin_doktor_ekle">Doktor Oluştur</button>
                </form>
            </div>
            <div>
                <div class="card">
                    <h3 class="card-title"><span class="card-icon">ℹ️</span> Bilgilendirme</h3>
                    <ul class="info-list">
                        <li>Doktor hesabı oluşturulduğunda <strong>kullanici</strong> ve <strong>doktor</strong> tablolarına kayıt eklenir.</li>
                        <li>Kayıt başarılı olursa doktor giriş yapabilir.</li>
                        <li>Hastalar doktor seçim ekranında sadece <strong>aktif</strong> doktorları görebilir.</li>
                    </ul>
                </div>
            </div>
        </div>
